...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("新的一生") action Start()

        else:

            textbutton _("回想") action ShowMenu("history")

            textbutton _("保存记忆") action ShowMenu("save")

        textbutton _("加载记忆恢复软盘") action ShowMenu("load")

        textbutton _("控制面板") action ShowMenu("preferences")
...
screen about():
    ...
    use game_menu(_("关于"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("使用了 Ren'Py 引擎制作，{a=https://www.renpy.org/}Ren'Py 引擎官网{/a} [renpy.version_only]\n\n[renpy.license!t]")
...
screen load():
    ...
    use file_slots(_("读取记忆恢复软盘"))


screen file_slots(title):

    default page_name_value = FilePageNameInputValue(pattern=_("第 {} 页"), auto=_("自动保存记忆"), quick=_("快捷保存记忆"))
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%Y-%m-%d %H:%M"), empty=_("软盘为空")):
            ...
            vbox:
                ...
                if config.has_sync:
                    if CurrentScreenName() == "save":
                        textbutton _("上传记忆到网络上"):
                            action UploadSync()
                            xalign 0.5
                    else:
                        textbutton _("下载网络上的记忆"):
                            action DownloadSync()
                            xalign 0.5
